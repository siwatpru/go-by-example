...
func main() {
	...
	// Here’s a non-blocking receive. If a value
	// is available on messages then select will
  ...
  // If a value is available on one of them the corresponding cases
  // will execute, otherwise the default case will be selected.
	select {
	case msg := <-messages:
		fmt.Println("received message", msg)
	case sig := <-signals:
